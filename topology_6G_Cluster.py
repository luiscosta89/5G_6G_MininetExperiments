...
def _start_host_owd_responder(host, port=UDP_PORT):
    """
    ...
    return host.popen(f'python3 -u -c \"{code}\"')
...
def udp_experiment(net, duration, clusters):
    ...
    Per packet (inside the loop), prints:
      ...
      Cluster Head staY sending <size> bytes to h1/h2/h3
    ...
    Requires:
      - UDP_PORT constant
      - video_traffic_model()
      - _start_ch_forwarder_tx_time_reply_src(ch, host_ip, UDP_PORT)
      - import time, re, random at module top
      - delays[], throughputs[] lists defined globally
    """
    ...
    # Start persistent CH forwarders (for all 3 hosts)
    ch_procs = []
    for ch in ch_nodes.values():
        if ch:
            for host in hosts:
                ch_procs.append(_start_ch_forwarder_tx_time_reply_src(ch, host.IP(), UDP_PORT))
        ...
        for cid, members in cluster_nodes.items():
            ...
            for sta in members:
                ...
                for host in hosts:
                    ...
                    print(f"Cluster Head {ch.name} sending {size} bytes to {host.name}")
                    ...
                    if sta_ms is None and ch_ms is None:
                        ...
                        if total_ms > 0:
                            thr_bps = ((size*8)/(total_ms/1000.0))/1000
...
def topology(duration=30):
    ...
    pairs_h1 = []  # (sta_node, sta_intf, h1_intf)
    for h1_intf in h1.intfList():
        ...
        if peer_node in stations:
            pairs_h1.append((peer_node, other, h1_intf))

    print("paired sta<->h1 links:", [(sta.name, si.name, hi.name) for sta, si, hi in pairs_h1])

    # ---------- Robust sta<->h2 pairing from h1 side ----------
    pairs_h2 = []  # (sta_node, sta_intf, h1_intf)
    for h2_intf in h2.intfList():
        if not hasattr(h2_intf, 'link') or h2_intf.link is None:
            continue
        other = h2_intf.link.intf1 if h2_intf.link.intf2 is h2_intf else h2_intf.link.intf2
        if other is None or not hasattr(other, 'node'):
            continue
        peer_node = other.node
        if peer_node in stations:
            pairs_h2.append((peer_node, other, h2_intf))

    print("paired sta<->h2 links:", [(sta.name, si.name, hi.name) for sta, si, hi in pairs_h2])

    # ---------- Robust sta<->h3 pairing from h1 side ----------
    pairs_h3 = []  # (sta_node, sta_intf, h1_intf)
    for h3_intf in h3.intfList():
        if not hasattr(h3_intf, 'link') or h3_intf.link is None:
            continue
        other = h3_intf.link.intf1 if h3_intf.link.intf2 is h3_intf else h3_intf.link.intf2
        if other is None or not hasattr(other, 'node'):
            continue
        peer_node = other.node
        if peer_node in stations:
            pairs_h3.append((peer_node, other, h3_intf))

    print("Oiiiiii!")
    print("paired sta<->h3 links:", [(sta.name, si.name, hi.name) for sta, si, hi in pairs_h3])    
    ...
    # UDP echo server inside hosts
    server_h1 = _start_host_owd_responder(h1, port=12345)
    server_h2 = _start_host_owd_responder(h2, port=54321)
    server_h3 = _start_host_owd_responder(h1, port=50000)
